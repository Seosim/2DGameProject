...
class Boss(Sprite):
    ...
    def MoveHand(self):
        ...
        if self.ldir > self.l_hand.posY + SPEED:
            self.l_hand.posY += SPEED
        elif self.ldir < self.l_hand.posY:
            self.l_hand.posY -= SPEED

        if self.rdir > self.r_hand.posY + SPEED:
            self.r_hand.posY += SPEED
        elif self.rdir < self.r_hand.posY:
            self.r_hand.posY -= SPEED
        else:
